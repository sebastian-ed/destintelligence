...
begin
  ...
  if v_member_count = 0 and v_user_count = 1 then
    ...
    if v_org is null then
      insert into public.organizations(name,plan)
      values('Municipalidad de Santa Fe','standard') returning id into v_org;
    end if;

    select id into v_dest from public.destinations
     where organization_id=v_org and name='Santa Fe'
     order by created_at asc limit 1;
    if v_dest is null then
      insert into public.destinations(organization_id,name,province,country)
      values(v_org,'Santa Fe','Santa Fe','Argentina') returning id into v_dest;
    end if;
    ...
    values(v_dest,v_org,'Secretaría de Turismo · Santa Fe','#315d4d','Santa Fe · Santa Fe · Argentina')
    ...
    if v_study is null then
      insert into public.studies(
        ...
        v_org,v_dest,'Estudio de demanda turística · Santa Fe',
        'Visitantes de Santa Fe durante el período de relevamiento.',
